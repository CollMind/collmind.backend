  ...
  private async simulateBudgetImpact(
    ...
    for (const envelope of envelopeMap.values()) {
      try {
        ...
        if (foundEnvelope) {
          // INV-B-009 / Z47: `budget_envelopes.available_amount` DÜŞTÜ
          // (bkz. `1814000000000-DropAvailableAmountFromBudgetEnvelopes.ts`)
          // — kolon hiçbir reserve/commit/release yolu tarafından
          // güncellenmeyen bayat bir kopyaydı. Bu ara-adımın ayrışma-alarmı
          // (Z45 §3: iki-kaynak okuma + karşılaştırma + throw) GÖREVİNİ
          // YAPTI — teşhis verisini o üretti (canlı ölçüm: 4 zarfın 2'si
          // ayrık, `Z47 §1`) — ve kolonla birlikte KALDIRILDI (`Z47 §2c`).
          // Geriye TEK KAYNAK kalıyor: `v_budget_summary` (allocated −
          // reserved(incl. COMMIT) − consumed, sorgu anında türetilir,
          // `K-2.2` ailesinin ruhu).
          ...
          // ⛔ `Number()` YOK — `BudgetSummaryView` `DecimalTransformer` ile
          // ZATEN `number` döndürür. Sarmalamak bir no-op olmakla kalmaz,
          // `null` girdide **sessiz `0`** üretir (`parseFiniteOnRead` `null`'ı
          // olduğu gibi döndürür) — `§2.5`, ve burası bir RAG yolu.
          const current = summary.availableAmount; // canonical, ledger-derived
          const after = current - envelope.thisUpload;

          // Status belirleme (RAG) — config-driven thresholds, tenant-scoped
          const allocated = summary.allocatedAmount;
