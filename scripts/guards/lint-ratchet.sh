...
case "${1:-}" in
  ...
  --ratchet)
    ...
    # VIOLATION / OUT OF SCOPE / NEW RULE / NEW file — same exit code (1),
    # different label (Z77 §1a emsali).
    if [ "$IMPROVED_COUNT" -gt 0 ]; then
      echo
      echo "!! [$GUARD_NAME] BASELINE STALE: $IMPROVED_COUNT (file,rule) pair(s) improved vs baseline but the baseline was not lowered." >&2
